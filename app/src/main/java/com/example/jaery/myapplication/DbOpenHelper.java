...
public class DbOpenHelper {
    ...
    public int find(int chap)
    {
        Cursor cursor= mDB.rawQuery("select * from Log",null);

        while(cursor.moveToNext()){
            if(cursor.getInt(0)==chap)
                return cursor.getInt(1);
        }
        return -1;
    }

    public void insert(int chap,int ind)
    {
       mDB.execSQL("INSERT INTO Log " + "VALUES ("+ chap+","+ind+");");
    }

    public void Update(int chap,int ind)
    {
        mDB.execSQL("UPDATE Log set IND="+ind+" where CHAP="+chap+";");
    }
